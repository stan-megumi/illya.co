function BattleUI(){
	...
	arrow:    {imageData: undefined, width: 50, height: 50, gi: -1},
	loadCount:  0,
	size:       0,
    };
...
BattleUI.prototype.setCharacterArrow=function(qi){ // qi: questindex gi: gridIndex
	...
	if (gi>-1) {
	    ...
	    this.context.putImageData(this.data.arrow.imageData,
				      origin[0]+25,origin[1]+100); // push data  
	}
    }
}

BattleUI.prototype.setCharacterDamages=function(qi,dmg){ // qi: questindex gi: gridIndex
    var ctx=this.context;
    var gi=this.data.gi[qi]-7;
    var size=this.data.card.width;
    var fontsize=35;
    var origin=this.canvasGrid.vec[gi];  // fetch position
    for (var i=0;i<7;i++) {
	var o=this.canvasGrid.vec[i];
	ctx.clearRect(o[0],o[1],size,size)
    }
    if (dmg<0) {
	ctx.fillStyle="red"
    }
	...
	ctx.fillText(dmg,origin[0]+(size/2-fontsize)/2,75);
	ctx.fillStyle="black";
    }
}
...
BattleUI.prototype.updateCharacterInfo=function(c,order){
    ...
    var hpRatio=attr.Hp*1.0/desc.Hp;
    context.clearRect(0,0,canvas.width,canvas.height);
    context.fillText(desc.name+" ("+desc.m_Name+")",bW,bH+=inc);
    context.fillText(desc.name+" ("+desc.m_Name+")",bW,bH+=inc);
    context.fillText("Hp: {0} ({1}%)".format(attr.Hp,(hpRatio*100).toFixed(2)),  bW,bH+=inc);
    ...
    this.deadCharacter(c.desc.QuestIndex,hpRatio<=0?0:hpRatio);
}
...
BattleUI.prototype.deadCharacter=function(qi,ratio){ // qi: questindex gi: gridIndex
    var gi=this.data.gi[qi];
    var data=this.data.card.imageData[qi].data;
    var len=data.length/4;
    if (ratio>1) {ratio=1;}
    var begin=Math.floor((ratio)*len);
...
BattleUI.prototype.appendCharacterGUI=function(c,order){
	...
	var  w=data.skill.width/len;
	var  h=data.skill.height;
	context.clearRect(0,0,w,h);
	var xshift=6;
	var scale=0.88;
	context.drawImage(images.frame, 0, 0, w, h);
	context.drawImage(images.ult, xshift, xshift, w*scale, h*scale);

	var xpos=0;
	for (i=1;i<len;i++){
	    context.drawImage(images.frame, xpos+=w, 0, w, h);
	    var type=skill.m_SkillType[i];
	    if (type>=0 && type<=4) {
		switch (c.desc.m_Element) {
		case 0:
		...
		context.fillRect(xpos+xshift, xshift, w*scale, h*scale);
		context.fillStyle="black"
	    }
	    context.drawImage(images[i], xpos+xshift, xshift, w*scale, h*scale);

//	    var img=context.getImageData(xpos+xshift, xshift, w*scale, h*scale);
//	    context.putImageData(img, xpos+xshift, xshift);

	}
	skills.imageData[c.desc.QuestIndex]=context.getImageData(0,0,w*4,h);
